        ...
        <div className="flex flex-col h-full bg-stone-50">
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                {messages.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full text-stone-400 space-y-3">
                        ...
                        <div className="text-xs text-stone-300 space-y-1 text-center">
                            <p>试试说：</p>
            ...
            <div className="p-4 border-t border-stone-200 bg-white">
                ...
                <div className="flex items-center gap-2 mb-3 pb-3 border-b border-stone-100">
                    <span className="text-[9px] font-bold text-stone-400 uppercase">快捷技能</span>
                    <button
                        onClick={() => onSendMessage('帮我质检一下')}
                        disabled={isProcessing}
                        className="px-2.5 py-1 bg-rose-50 text-rose-600 rounded-lg text-[10px] font-bold hover:bg-rose-100 transition-all disabled:opacity-40"
                    >
                ...
                <div className="flex items-center gap-2 bg-stone-50 rounded-xl p-2">
                    <input
                        ...
                        placeholder="输入消息或指令..."
                        className="flex-1 bg-transparent border-none text-sm outline-none placeholder:text-stone-400"
                        disabled={isProcessing}
                    />
                    <button
                        onClick={handleSend}
                        disabled={!input.trim() || isProcessing}
                        className="p-2 bg-black text-white rounded-lg disabled:opacity-40 transition-all hover:bg-stone-800"
                    >
                        {isProcessing ? (
            ...
            <div className="bg-white border border-stone-200 rounded-2xl p-4 shadow-sm">
                <div className="flex items-center gap-2 mb-3">
                    <div className="p-1.5 bg-amber-100 rounded-lg">
                        <Icons.Wand2 size={14} className="text-amber-600" />
                    </div>
                    ...
                    {message.applied && (
                        <span className="text-[10px] px-2 py-0.5 bg-emerald-100 text-emerald-600 rounded-full font-bold">已应用</span>
                    )}
                </div>

                {message.content && (
                    <div className="text-sm text-stone-600 mb-3 prose prose-sm max-w-none">
                ...
                <div className="space-y-2">
                    {message.suggestions.map((suggestion, idx) => (
                        ...
                        >
                            ...
                            />
                            <span className="text-xs text-stone-600">{suggestion}</span>
                        </label>
                    ))}
                </div>
                ...
                {!message.applied && message.selectedIndices && message.selectedIndices.length > 0 && (
                    <button
                        onClick={handleApply}
                        disabled={isApplying}
                        className="mt-3 w-full py-2 bg-black text-white rounded-xl text-xs font-bold hover:bg-stone-800 transition-all disabled:opacity-60 flex items-center justify-center gap-2"
                    >
                        {isApplying ? (
                            <>
